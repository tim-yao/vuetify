...
test('VDialog', ({ mount, compileToFunctions, functionalContext }) => {
  ...
  // TODO: broken by vue bug https://github.com/vuejs/vue/issues/8350
  it.skip('should open dialog on activator click', async () => {
    const input = jest.fn()
    const component = {
      render (h) {
        return h('div', [h(
    ...
    wrapper.first('.dialog__activator').trigger('click')
